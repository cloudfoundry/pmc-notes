...
- Unpinned cf-cli release now that the cf app details have been re-added (this unblocks a few CATs stories that were dependent on the latest cf-cli version)


#### `cf-acceptance-tests`
- Fix issue with Windows tests so the 'include_internet_dependent' flag is respected
- Reduce flakiness around broker healthchecks by calling `v3-set-health-check` (acknowledge this is a short-term solution given cf-cli V7 is under development)
